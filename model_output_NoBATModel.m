...
% Solidoro (2018) without the BAT mechanism. This function takes in parameter values for the 
...
function model_predictions = model_output_NoBATModel(parameter_vals,dataset)
...
p.k3b_c=parameter_vals(13);  p.sigma=parameter_vals(14);
...
% 3. Final ODEs
dDIC_coel_dt = (J_sw_coel_DIC - J_coel_ecm_DIC - J_CO2 - p.Pg + p.R)/p.h_coel; 
dTA_coel_dt = (- 2*J_CaATPase + J_sw_coel_TA - J_coel_ecm_TA)/p.h_coel;          
dCa_coel_dt = (-J_CaATPase + J_sw_coel_Ca - J_coel_ecm_Ca)/p.h_coel;                   
dDIC_ecm_dt = (J_coel_ecm_DIC + J_CO2 - J_CaCO3)/p.h_ecm;                  
dTA_ecm_dt = (2*J_CaATPase + J_coel_ecm_TA - 2*J_CaCO3)/p.h_ecm;           
