...
% Solidoro (2018). This function takes in parameter values for the 20
...
function model_predictions = model_output_OriginalModel(parameter_vals,dataset)


%% Calcification model

%getting parameter values from the inputs:
p.k_CO2 = parameter_vals(1); p.k_pp = parameter_vals(2); 
p.s = parameter_vals(3); p.alpha=parameter_vals(4); 
p.beta=parameter_vals(5); p.v_H_c=parameter_vals(6); 
p.E0_c=parameter_vals(7); p.k1f_c=parameter_vals(8); 
p.k2f_c=parameter_vals(9); p.k3f_c=parameter_vals(10); 
p.k1b_c=parameter_vals(11); p.k2b_c=parameter_vals(12); 
p.k3b_c=parameter_vals(13); p.E0_b=parameter_vals(14); 
p.k1f_b=parameter_vals(15); p.k2f_b=parameter_vals(16); 
p.k3f_b=parameter_vals(17); p.k1b_b=parameter_vals(18); 
p.k2b_b=parameter_vals(19); p.k3b_b=parameter_vals(20); 
p.sigma=parameter_vals(21);
...
J_CO2 = p.k_CO2 * (CO2_coel-CO2_ecm);                                                      
...
% 3. Final ODEs
dDIC_coel_dt = (-J_BAT + J_sw_coel_DIC - J_coel_ecm_DIC - J_CO2 - p.Pg + p.R)/p.h_coel; 
...
dDIC_ecm_dt = (J_BAT + J_coel_ecm_DIC + J_CO2 - J_CaCO3)/p.h_ecm;                  
